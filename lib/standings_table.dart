...
/// Signature for [StandingsDataColumn.onSort] callback.
typedef StandingsDataColumnSortCallback = void Function(int columnIndex, bool ascending);

/// Column configuration for a [StandingsTable].
///
/// One column configuration must be provided for each column to
/// display in the table. The list of [StandingsDataColumn] objects is passed
/// as the `columns` argument to the [new StandingsTable] constructor.
@immutable
class StandingsDataColumn {
  /// Creates the configuration for a column of a [StandingsTable].
  ///
  /// The [label] argument must not be null.
  const StandingsDataColumn({
    @required this.label,
    this.tooltip,
    this.numeric = false,
    this.onSort,
  ...
  ///
  /// See [StandingsTable.sortColumnIndex] and [StandingsTable.sortAscending].
  final StandingsDataColumnSortCallback onSort;
...
/// Row configuration and cell data for a [StandingsTable].
///
/// One row configuration must be provided for each row to
/// display in the table. The list of [StandingsDataRow] objects is passed
/// as the `rows` argument to the [new StandingsTable] constructor.
///
/// The data for this row of the table is provided in the [cells]
/// property of the [StandingsDataRow] object.
@immutable
class StandingsDataRow {
  /// Creates the configuration for a row of a [StandingsTable].
  ///
  /// The [cells] argument must not be null.
  const StandingsDataRow({
    this.key,
    this.selected = false,
  ...
  /// Creates the configuration for a row of a [StandingsTable], deriving
  /// the key from a row index.
  ///
  /// The [cells] argument must not be null.
  StandingsDataRow.byIndex({
    int index,
    this.selected = false,
    this.onSelectChanged,
    @required this.cells,
  ...
  /// table.
  final List<StandingsDataCell> cells;

  bool get _debugInteractive => onSelectChanged != null || cells.any((StandingsDataCell cell) => cell._debugInteractive);
}

/// The data for a cell of a [StandingsTable].
///
/// One list of [StandingsDataCell] objects must be provided for each [StandingsDataRow]
/// in the [StandingsTable], in the [new StandingsDataRow] constructor's `cells`
/// argument.
@immutable
class StandingsDataCell {
  /// Creates an object to hold the data for a cell in a [StandingsTable].
  ///
  ...
  static final StandingsDataCell empty = StandingsDataCell(Container(width: 0.0, height: 0.0));
  ...
  /// [StandingsDataRow.onSelectChanged] is provided).
...
/// [PaginatedStandingsTable] which automatically splits the data into
/// multiple pages.
// TODO(ianh): Also suggest [ScrollingStandingsTable] once we have it.
///
/// See also:
///
///  * [StandingsDataColumn], which describes a column in the data table.
///  * [StandingsDataRow], which contains the data for a row in the data table.
///  * [StandingsDataCell], which contains the data for a single cell in the data table.
///  * [PaginatedStandingsTable], which shows part of the data in a data table and
///    provides controls for paging through the remainder of the data.
///  * <https://material.io/design/components/data-tables.html>
class StandingsTable extends StatelessWidget {
  /// Creates a widget describing a data table.
  ///
  /// The [columns] argument must be a list of as many [StandingsDataColumn]
  ...
  ///
  /// The [rows] argument must be a list of as many [StandingsDataRow] objects
  ...
  /// not be null.
  ///
  /// Each [StandingsDataRow] object in [rows] must have as many [StandingsDataCell]
  /// objects in the [StandingsDataRow.cells] list as the table has columns.
  ///
  ...
  StandingsTable({
    ...
    @required this.rows,
       ...
       assert(rows != null),
       assert(!rows.any((StandingsDataRow row) => row.cells.length != columns.length)),
       _onlyTextColumn = _initOnlyTextColumn(columns),
       super(key: key);

  /// The configuration and labels for the columns in the table.
  final List<StandingsDataColumn> columns;
  ...
  ///
  /// If this is null, then the [StandingsDataRow.onSelectChanged] callback of
  /// every row in the table is invoked appropriately instead.
  ///
  /// To control whether a particular row is selectable or not, see
  /// [StandingsDataRow.onSelectChanged]. This callback is only relevant if any
  /// row is selectable.
  ...
  final List<StandingsDataRow> rows;

  // Set by the constructor to the index of the only Column that is
  // non-numeric, if there is exactly one, otherwise null.
  final int _onlyTextColumn;
  static int _initOnlyTextColumn(List<StandingsDataColumn> columns) {
    int result;
    for (int index = 0; index < columns.length; index += 1) {
      final StandingsDataColumn column = columns[index];
      if (!column.numeric) {
        if (result != null)
          return null;
        result = index;
  ...
  bool get _debugInteractive {
    return columns.any((StandingsDataColumn column) => column._debugInteractive)
        || rows.any((StandingsDataRow row) => row._debugInteractive);
  }
  ...
  void _handleSelectAll(bool checked) {
    if (onSelectAll != null) {
      ...
      for (StandingsDataRow row in rows) {
  ...
  Widget _buildHeadingCell({
    ...
    label = Container(
      ...
      alignment: alignment != null? alignment : (numeric ? Alignment.centerRight : AlignmentDirectional.centerStart),
  ...
  Widget build(BuildContext context) {
    ...
    final BoxDecoration _kSelectedDecoration = BoxDecoration(
      border: Border(bottom: Divider.createBorderSide(context, width: 1.0, color: Colors.white54)),
    ...
    final bool showCheckboxColumn = rows.any((StandingsDataRow row) => row.onSelectChanged != null);
    final bool allChecked = showCheckboxColumn && !rows.any((StandingsDataRow row) => row.onSelectChanged != null && !row.selected);
    ...
    for (int dataColumnIndex = 0; dataColumnIndex < columns.length; dataColumnIndex += 1) {
      final StandingsDataColumn column = columns[dataColumnIndex];
      ...
      rowIndex = 1;
      for (StandingsDataRow row in rows) {
        final StandingsDataCell cell = row.cells[dataColumnIndex];
